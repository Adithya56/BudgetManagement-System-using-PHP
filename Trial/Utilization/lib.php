...
if (isset($_POST['submit'])) {
    $aquery = "SELECT Amount FROM Allocating where Id='c'";
    $result = $conn->query($aquery);
    $alib = mysqli_fetch_array($result)[0];

    $uquery = "SELECT Amount FROM utilized where Id='c'";
    $result1 = $conn->query($uquery);
    $ulib = mysqli_fetch_array($result1)[0];
    ...
    if ($curr+$ulib > $alib) {
        ...
        $query = "update utilized set Amount=($curr+$ulib) where Id='c'";
        $conn->query($query);
        $n1=$_POST['number0'];
		$n2=$_POST['number1'];
		$n3=$_POST['number2'];
		...
		$query =$conn->prepare("insert into library1 (books,magazine,onlineser,total,disc,date) values(?,?,?,?,?,?)");

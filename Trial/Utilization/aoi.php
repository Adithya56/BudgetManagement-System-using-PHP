...
if(isset($_POST['submit'])){
    $aquery = "SELECT Amount FROM Allocating where Id='l'";
    $result = $conn->query($aquery);
    $aaoi=mysqli_fetch_array($result)[0];

    $uquery = "SELECT Amount FROM utilized where Id='l'";
    $result1 = $conn->query($uquery);
    $uaoi=mysqli_fetch_array($result1)[0];
    ...
    if ($curr+$uaoi>$aaoi) {
        ...
        $query = "update utilized set Amount=($curr+$uaoi) where Id='l'";
        $conn->query($query);
        $n1=$_POST['number0'];
		$n2=$_POST['number1'];
		$n3=$_POST['text'];
        $time=date("d-m-Y H:i:s");
		$query =$conn->prepare("insert into others1 (other,total,disc,date) values(?,?,?,?)");
		$query->bind_param("iiss",$n1,$n2,$n3,$time);
		$query->execute();
    }
}
$conn->close(); 
...
<head>
    ...
    <script>
        function sum(){
            ...
            document.forms["myform"]["number1"].value=total;
        }
    </script>
</head>
<body>

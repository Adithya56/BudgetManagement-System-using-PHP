...
if (isset($_POST['submit'])) {
    $aquery = "SELECT Amount FROM Allocating where Id='b'";
    $result = $conn->query($aquery);
    $aequi = mysqli_fetch_array($result)[0];

    $uquery = "SELECT Amount FROM utilized where Id='b'";
    $result1 = $conn->query($uquery);
    $uequi = mysqli_fetch_array($result1)[0];
    ...
    if ($curr+$uequi > $aequi) {
        ...
        $query = "update utilized set Amount=($curr+$uequi) where Id='b'";
        $conn->query($query);
        $n1=$_POST['number0'];
	    $n2=$_POST['number1'];
	    $n3=$_POST['number2'];
	    ...
	    $query =$conn->prepare("insert into equipment1(newli,renewal,total,disc,date) values(?,?,?,?,?)");
...
<body>
    <form action="#" method="POST" name="myform">
        <table cellpadding='8' align='center' style="margin-top:40px;">
            <tr>
                <th style="font-size:20px;">Equipment</th>
            </tr>
            <tr>
                <td><label>New Licenses</label><br></td>
                <td><input type='number' name='number0' onkeyup="sum();" /></td>
            </tr>
            <tr>
                <td><label>Renewals</label><br></td>

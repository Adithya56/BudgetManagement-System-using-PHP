...
if(isset($_POST['n10'])){
	if(!empty($_POST['number0']) && !empty($_POST['number1']) && !empty($_POST['number2'])&& !empty($_POST['number3'])&& !empty($_POST['number4'])&& !empty($_POST['number5'])&& !empty($_POST['number6'])&& !empty($_POST['number7'])&& !empty($_POST['text'])){
		...
		$n8=$_POST['number7'];
		$n9=$_POST['text'];
		$query =$conn->prepare("insert into maintainance (labcon,repair,opcost,stationary,mcost,printerco,deptcost,total,disc) values(?,?,?,?,?,?,?,?,?)");
		$query->bind_param("iiiiiiiis",$n1,$n2,$n3,$n4,$n5,$n6,$n7,$n8,$n9);
		$query->execute();
        if($query){
		    $ins =$conn->prepare("update tab set d =(?)");
		    $ins->bind_param("i",$n8);
		    $ins->execute();
	}
}
